...
def load(variant: str):
    ...
    index_path = script_dir / f"eecs_ind_{variant}.faiss"
    json_path  = script_dir / f"data_storage_{variant}.json"

    df    = pd.read_json(str(json_path), orient="records")
    model = SentenceTransformer('BAAI/bge-small-en-v1.5')
    ind   = faiss.read_index(str(index_path))
    bm25  = BM25Okapi([[word.lower() for word in doc.split() if len(word) > 2]
                       for doc in df['txt'].tolist()])
...
def main():
    ...
    variant   = sys.argv[3]  # e.g. "300-60"

    model, ind, df, bm25 = load(variant)
    ...
    print(f"[{variant}] Done → {pred_path}")
